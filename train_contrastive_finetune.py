from networks import Encoder3D,Classifier3D,R3D_18_Encoder,R3D_18_Encoder_Split, \
    R3D_18_Encoder_Split_ver2, R3D_18_Encoder_Split_ver2_finetune
...
def finetune(cv=0, batch_size=4, log_path_=None, save_checkpoint=None, model_path=None):
    # training protocol
    batch_size = batch_size
    lr = 0.001
    con_temperature = 0.05
    ...
    train_rupture_dataset = RupturedDataset_ver2(s_img_ids=list(unruptured_splits[order]['train']),
                                                 ...
                                                 aug=True, train=True, attach_clinic=True)
    ...
    net = R3D_18_Encoder_Split_ver2_finetune(model_ckpt=model_path, linear=2, frozen=False, attach_clinic=False)
    net.cuda()
    ...
    for epoch in range(TOTAL_EPOCHS):
        it = -1
        net.train()
        for img, trans_img, label, idx, clinic_feature in train_loader:
            loss1_value = 0.
            loss2_value = 0.
            it += 1
            if torch.cuda.is_available():
            ...
            finetune_logits = net(trans_img, clinic_feature=None)

            loss1 = classifier_criterion(finetune_logits, label.float())
        ...
        with torch.no_grad():
            ...
            for img, trans_img, label, idx, clinic_feature in val_loader:
                if torch.cuda.is_available():
                    ...
                    clinic_feature = clinic_feature.float().cuda(non_blocking=True)
                logits = net(img, clinic_feature=None)
            ...
            if auc > best_auc:
                best_auc = auc
                torch.save({'model': net.state_dict()}, CHECKPOINT_PATH + '/finetune_checkpoint_cv{}_epoch{}_bs{}.pth'.format(cv, epoch, batch_size))
                print_log('save best, auc!', log)
            if acc > best_acc:
                best_acc = acc
                torch.save({'model': net.state_dict()}, CHECKPOINT_PATH + '/finetune_checkpoint_cv{}_epoch{}_bs{}.pth'.format(cv, epoch, batch_size))
                print_log('save best, acc!', log)



if __name__ == '__main__':
    model_path='/home/xxx'
    finetune(4, batch_size=4, log_path_='./logs_unsupervised_finetune',
             save_checkpoint='./checkpoints_unsupervised_finetune',
